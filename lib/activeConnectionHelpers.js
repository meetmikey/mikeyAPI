      ...
      if (err) {
        ...
        foundConn.save (function (err) {
          if (err) {
            ...
            winston.doInfo ('Adding active connection message to queue because mikeyMailTS hasn\'t updated timestamp in awhile');
            sqsConnect.addMessageToMailActiveConnectionQueue (user, function(err) {
              if ( err ) {
