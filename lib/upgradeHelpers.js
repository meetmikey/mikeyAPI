...
exports.upgradeUser = function(userEmail, billingPlan, stripeToken, callback) {
	...
	UserModel.findOne( {email: userEmail}, function(mongoErr, foundUser) {
		if ( err ) {
			...
			upgradeHelpers.getStripeCustomerId( foundUser, function(err, stripeCustomerId) {
				if ( err ) {
					callback( err );
					...
					userUpgrade.save( function(mongoErr) {
						if ( mongoErr ) {
							...
							upgradeHelpers.createCharge( foundUser, stripeToken, function(err) {
								if ( err ) {
									callback( err );
							    ...
							    UserModel.findByIdAndUpdate( foundUser._id, updateData, function(mongoErr, updatedUser) {
							    	if ( mongoErr ) {
							    		...
							    		upgradeHelpers.markUserUpgradeStatus( userUpgrade, 'success', callback );
							    	}
							    });
								}
							});
...
exports.createCharge = function( user, callback ) {
	...
	stripe.charges.create( stripeChargeData, function( err, customer ) {
		if ( err ) {
