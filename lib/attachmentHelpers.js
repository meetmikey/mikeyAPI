  ...
  AttachmentModel.find({userId:userId})
    ...
    .exec (function (err, foundAttachments) {
      if ( err ) {
        ...
        attachmentHelpers.addSignedURLs(foundAttachments);
        attachmentHelpers.sortByDate(foundAttachments);
    ...
    .exec (function (err, foundAttachments) {
      if ( err ) {
        ...
        foundAttachments.forEach (function (attachment) {
          if (String(attachment.userId) != String(userId)) {
            console.log (attachment.userId)
            console.log (userId)
            console.log (attachment.userId  == userId)
            winston.doError ('ACL PROBLEM', {userId : userId});
          }
        })*/

        console.log ('before', foundAttachments)

        attachmentHelpers.addSignedURLs(foundAttachments);
        attachmentHelpers.sortByDate(foundAttachments);
        console.log ('after', foundAttachments)
