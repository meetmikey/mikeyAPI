...
exports.doSearch = function (query, userId, from, size, searchCallback) {
  ...
  if (filteredQuery.query == "") {
    searchCallback (null, {'attachments' : [], 'links': []});
    return;
  }


  elasticSearchClient.search(conf.elasticSearch.indexName, 'resource', 
...
exports.getMustClauses = function (userId, filters) {
  console.log (filters)
  var must = [ 
    {
      "term" : {
...
/*      var tq = { 
        terms : {
          "authorEmail" : queryText,
          "authorName" : queryText
        },
        "minimum_match" : 1
      }
      */
...
exports.filterGmailSearchQuery = function (query) {
  ...
  if (decodedQuery.match(filenameRegex)) {
    var filename = decodedQuery.match(filenameRegex)[1];
